...
class BxiExample(Node):
    ...
    def linear_interpolation(self,current_angles, target_angles, points):
        ...
        path = current_angles + np.outer(alphas, target_angles - current_angles)
    ...
    def timer_callback(self):
        ...
        if self.step == 1:
            ...
            #arm
            q[(3+12):(3+12+4)] = joint_nominal_pos[(3+12):(3+12+4)]
            q[(3+12+4):(3+12+8)] = joint_nominal_pos[(3+12+7):(3+12+11)]
            
            dq[(3+12):(3+12+4)] = [0,0,0,0]
            dq[(3+12+4):(3+12+8)] = [0,0,0,0]
            ...
            # #arm
            # qpos[(3+12):(3+12+7)] =[0.8,0.3,-0.1,-0.2, 0.0, 0.0,0.0]
            # qpos[(3+12+7):(3+12+14)] =[0.8,-0.3,0.1,-0.2, 0.0, 0.0,0.0]
            qpos[(3+12):(3+12+7)] =self.l_arm
            qpos[(3+12+7):(3+12+14)] =self.r_arm 
    ...
    def timer_callback2(self):
        # self.l_arm = [0.8,0.3,-0.1,-0.2, 0.0, 0.0,0.0]
        if self.step ==2:
            print("arm_start")
            time.sleep(4)
            arm_start=0
            while True:
                if arm_start==0:
                    # 插值到预设位置
                    start_pos = np.array([0.5,0.3,-0.1,-0.2, 0.0, 0.0,0.0])
                    end_pos = np.array([0.5,1.57,-0.1,-0.2, 0.0, 0.0,0.0])
                    actions = self.linear_interpolation(start_pos, end_pos, 100)
                    ...
                    # 插值到预设位置
                    start_pos = end_pos
                    end_pos = np.array([0.0,1.57,-0.1,1.57, 0.0, 0.0,0.0])
                    actions = self.linear_interpolation(start_pos, end_pos, 100)
                    for action in actions:
                        self.l_arm = action
                        time.sleep(0.01)
                ...
                start_pos = end_pos
                end_pos = np.array([0.0,0.3,-0.1,1.57, 0.0, 0.0,0.0])
                actions = self.linear_interpolation(start_pos, end_pos, 100)
                for action in actions:
                    self.l_arm = action
                    time.sleep(0.01)
