...
class LocoMotionPolicy:
    ...
    def __init__(self, model_onnx_path: str):
        ...
        Usage:
            ##1.初始化模型
            self.amp_policy = LocoMotionPolicy("path/to/model.onnx")
        ...
        self.num_obs = 1020  # 102 * 10 (observation dimension * history length)

        self.obs_history_len = 10
        
        self.single_obs_dim = 3 + 3 + 3 + self.num_actions*3 +2+2+2 #102
    ...
    def initialize_model(self, onnx_path):
        ...
        #     (self.single_obs_dim * self.obs_history_len,), dtype=np.float32
        # )# 观测历史750
    ...
    def compute_observation(self,qj, dqj, quat, omega, cmd_vel):
        ...
        # self.obs_history = np.roll(self.obs_history, shift=-self.single_obs_dim) #滚动历史观测
        # self.obs_history[-self.single_obs_dim :] = single_obs.copy() #添加最新观测
        # # return np.clip(self.obs_history, -self.cfg.sim.clip_observations, self.cfg.sim.clip_observations) #裁剪观测值100
        # return np.expand_dims(self.obs_history, axis=0)
